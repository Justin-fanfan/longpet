...
void KwsProcessAdapter::processLine(const QByteArray& line)
{
    ...
    if (event == QStringLiteral("vad_stats")) {
        qInfo().noquote() << QStringLiteral(
            "KWS VAD window_ms=%1 inference_ratio=%2%% noise_db=%3 "
            "threshold_db=%4 speech_starts=%5 active=%6")
            .arg(object.value(QStringLiteral("window_ms")).toInteger())
            .arg(object.value(QStringLiteral("inference_ratio")).toDouble(), 0, 'f', 1)
            .arg(object.value(QStringLiteral("noise_db")).toDouble(), 0, 'f', 1)
            .arg(object.value(QStringLiteral("threshold_db")).toDouble(), 0, 'f', 1)
            .arg(object.value(QStringLiteral("speech_starts")).toInt())
            .arg(object.value(QStringLiteral("active")).toBool());
        return;
    }
    if (event == QStringLiteral("paused")) {
        if (!m_ready || m_stopping || !m_pauseRequested || m_waitingCommandId <= 0
            || m_waitingCommand != QStringLiteral("pause")
            || object.value(QStringLiteral("command_id")).toInteger()
                != m_waitingCommandId)
            return;
        m_waitingCommand.clear();
        m_paused = true;
    ...
    if (event == QStringLiteral("resumed")) {
        if (!m_ready || m_stopping || m_waitingCommandId <= 0
            || m_waitingCommand != QStringLiteral("resume")
            || object.value(QStringLiteral("command_id")).toInteger() != m_waitingCommandId)
            return;
        m_waitingCommand.clear();
        m_paused = false;
        if (m_pauseRequested) {
