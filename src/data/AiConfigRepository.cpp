...
AiConfiguration AiConfigRepository::load(QString* error) const
{
    ...
    configuration.voice.vadThresholdDb = environmentDouble(
        "LONGPET_VOICE_VAD_THRESHOLD_DB", boundedDouble(
            settings, voicePrefix + QStringLiteral("vad_threshold_db"), -55.0));
    configuration.voice.vadNoiseRatio = environmentDouble(
        "LONGPET_VOICE_VAD_NOISE_RATIO", boundedDouble(
            settings, voicePrefix + QStringLiteral("vad_noise_ratio"), 2.0));
