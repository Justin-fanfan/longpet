...
SPLIT_VIDEOS: dict[str, list[str]] = {
    # Every file occurs exactly once. Splits are by complete source video.
    "train": [
        "1场景1人绿右.mp4", "1场景1人绿坐.mp4", "1场景1人绿左.mp4", "1场景1人绿正.mp4",
        "2场景1人绿后.mp4", "2场景1人绿正.mp4", "2场景1人绿右.mp4",
        "3场景1人后白.mp4", "3场景1人白光.mp4", "3场景1人白右.mp4", "3场景1人白左.mp4",
        "3场景1人白正.mp4", "3场景1人绿光.mp4", "3场景1人绿后.mp4", "3场景1人绿坐.mp4",
        "3场景1人绿左.mp4", "3场景1人绿正.mp4", "3场景1人绿远.mp4", "3场景1人黑暗.mp4",
        "3场景1人黑右.mp4", "3场景1人黑左.mp4", "3场景1人黑远.mp4", "3场景1人黑坐.mp4",
        "3场景1人白远.mp4",
    ],
    "val": [
        "1场景1人绿后.mp4", "2场景1人绿坐.mp4", "3场景1人白暗.mp4",
        "3场景1人白正‘.mp4", "3场景1人绿右.mp4", "3场景1人黑光.mp4",
    ],
    "test": [
        "1场景1人绿远.mp4", "2场景1人绿左.mp4", "3场景1人坐白.mp4",
        "3场景1人绿暗.mp4", "3场景1人黑后.mp4", "3场景1人黑正.mp4",
    ],
}

EXCLUDED_FILES = {
    "WIN_20260909_22_10_53_Pro.mp4": "0.91-second 1280x720 near-black clip; kept in raw, excluded from dataset",
}
...
def assign_outputs(records: list[FrameRecord], video_info: dict[str, VideoInfo],
    ...
    for video, rows in by_video.items():
        ...
        for rec in rows:
            ...
            if override is not None and not rec.bad_frame:
                ...
                if rec.teacher_status == "positive":
                    ...
                    rec.source_kind == "video"
                    and rec.teacher_status == "no_detection"
                    and positives
                    and (rec.time_seconds <= 0.8 or rec.time_seconds >= duration - 0.8)
                ):
                    rec.teacher_status = "negative"
                    rec.review_reason = "edge_frame_without_person_detection"
                else:
...
def write_report(output: Path, infos: list[VideoInfo], records: list[FrameRecord],
    ...
    report: list[str] = []
    report.append("# LongPet 实拍 Person-only 数据集质量报告\n")
    report.append("本报告由 `prepare_longpet_person_dataset.py` 生成。视频级划分，任何一个视频的帧只进入一个 split，避免相邻帧泄漏；standalone 图片作为独立素材审计。\n")
    ...
    report.append("- 质量规则：Laplacian 方差 <2 或灰度均值 <4 的抽样帧剔除；teacher 置信度 <0.20、疑似漏检帧进入 review，不写入训练集。低光但仍有结构的 scene3 帧保留。\n")
    aggregate = Counter()
    ...
    report.append("- teacher 标签：普通帧只保留最高置信度 person 框；8 张贴近镜头的部分人体帧经过人工审核并使用保守覆盖框；所有框裁剪到图像边界并转换为 YOLO 归一化格式。\n")
    report.append("- negative：视频仅保留有同视频正样本且处于视频首尾 0.8 秒、teacher 未检测到人的帧；standalone 图片即使 teacher 未检测到人也不会自动当作 negative。\n")
    report.append("\n## 排除素材\n")
...
def main() -> int:
    ...
    write_yaml(output)
    manifest = {
        "dataset": "longpet-realworld-person-v1",
        "class_names": ["person"],
        "raw_dir": str(raw_dir),
        ...
        "videos": [asdict(v) | {"scene": compact_scene_name(v.filename)} for v in infos],
