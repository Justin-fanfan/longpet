...
TRACKING_BENCHMARK="${LONGPET_VISION_TRACKING_BENCHMARK:-0}"
TRACKER_INTERVAL_MS="${LONGPET_VISION_TRACKER_INTERVAL_MS:-100}"
CORRECTION_MS="${LONGPET_VISION_DETECTOR_CORRECTION_MS:-8000}"
SEARCH_INTERVAL_MS="${LONGPET_VISION_SEARCH_INTERVAL_MS:-250}"
LOST_INTERVAL_MS="${LONGPET_VISION_LOST_INTERVAL_MS:-250}"
...
if [[ "${TRACKING_BENCHMARK}" == "1" || "${TRACKING_BENCHMARK}" == "true" ]]; then
    BENCHMARK_ARGUMENTS+=(
        --tracking
        --tracker-interval-ms "${TRACKER_INTERVAL_MS}"
        --correction-ms "${CORRECTION_MS}"
        --search-interval-ms "${SEARCH_INTERVAL_MS}"
        --lost-interval-ms "${LOST_INTERVAL_MS}"
    )
fi
