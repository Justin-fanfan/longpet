...
def main():
    ...
    # component root. The bridge overlays LongPet's command vocabulary while the
    # component's acoustic runtime stays independent of C++ business policy.
    ...
    stats_audio_samples = 0
    stats_inference_samples = 0
    stats_speech_starts = 0
    ...
    try:
        ...
        while True:
            ...
            stats_audio_samples += len(block)
            chunks = vad.accept(block)
            for chunk in chunks:
                stats_inference_samples += len(chunk.audio)
                if chunk.speech_started:
                    stats_speech_starts += 1
                ...
                if chunk.speech_ended:
                    detector.reset_stream()
            now = time.monotonic()
            if now - stats_started >= 30.0:
                ratio = (100.0 * stats_inference_samples / stats_audio_samples
                         if stats_audio_samples else 0.0)
                emit("vad_stats", window_ms=round((now - stats_started) * 1000),
                     inference_ratio=round(ratio, 1),
                     noise_db=round(vad.noise_floor_db, 1),
                     threshold_db=round(vad.effective_threshold_db, 1),
                     speech_starts=stats_speech_starts,
                     active=vad.active)
                stats_started = now
                stats_audio_samples = 0
                stats_inference_samples = 0
